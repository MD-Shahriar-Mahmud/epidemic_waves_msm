...
from funcs.format_label import format_label
...
def draw_heatmap(matrix,x_range,y_range,x_param_name,y_param_name,subfolder_name,global_max=None,global_min=None):
    ...
    if x_param_name == 'c':
        ax.set_xlabel(f"{format_label(x_param_name)} (% of total population)")
    else:
        ax.set_xlabel(format_label(x_param_name))
    if y_param_name == 'c':
        ax.set_ylabel(f"{format_label(y_param_name)} (% of total population)")
    else:
        ax.set_ylabel(format_label(y_param_name))    
    ...
    if y_param_name == 'k':
        ...
        ax.set_ylabel(f"sensitivity of response ({format_label(y_param_name)})")
        h_text = 'high'
        l_text = 'low'
    elif y_param_name == 'c':
        y_axis_ticks_vals = np.array([f"{val:.1f}" for val in y_axis_ticks_vals])
        ax.set_ylabel(f"response threshold \nin % of total population ({format_label(y_param_name)})")
        h_text = 'high'
        l_text = 'low'
    elif y_param_name == 'beta':
        y_axis_ticks_vals = np.array([f"{val:.2f}" for val in y_axis_ticks_vals])
        ax.set_ylabel(f"transmission rate ({format_label(y_param_name)})")
        h_text = 'fast'
        l_text = 'slow'
    elif y_param_name == 'gamma':
        y_axis_ticks_vals = np.array([f"{val:.2f}" for val in y_axis_ticks_vals])
        ax.set_ylabel(f"recovery rate ({format_label(y_param_name)})")
        h_text = 'fast'
        l_text = 'slow'
    else:
        y_axis_ticks_vals = np.array([f"{val:.2f}" for val in y_axis_ticks_vals])
    ...
    ax.set_xlabel(f"delay time ({format_label(x_param_name)})")
